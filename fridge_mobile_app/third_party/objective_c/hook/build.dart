...
String firstLineOfStdout(String cmd, List<String> args) {
  ...
  if (result.exitCode == 0 && lines.isNotEmpty) {
    return lines.first;
  }

  final fallbackEnv = Map<String, String>.from(Platform.environment);
  fallbackEnv.putIfAbsent(
    'DEVELOPER_DIR',
    () => '/Applications/Xcode.app/Contents/Developer',
  );
  final fallback = Process.runSync(cmd, args, environment: fallbackEnv);
  final fallbackLines = (fallback.stdout as String)
      .split('\n')
      .where((line) => line.isNotEmpty);
  if (fallback.exitCode == 0 && fallbackLines.isNotEmpty) {
    return fallbackLines.first;
  }

  final originalError = (result.stderr as String).trim();
  final fallbackError = (fallback.stderr as String).trim();
  throw StateError(
    'Failed to run `$cmd ${args.join(" ")}`. '
    'originalExit=${result.exitCode}, fallbackExit=${fallback.exitCode}, '
    'originalStderr="$originalError", fallbackStderr="$fallbackError"',
  );
}
